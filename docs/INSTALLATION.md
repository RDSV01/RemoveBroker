...
Lancez `RemoveBroker-1.0.0-installateur.exe`.
...
```bash
chmod +x RemoveBroker-1.0.0-x64.AppImage
./RemoveBroker-1.0.0-x64.AppImage
```

Ou installez le paquet Debian:

```bash
sudo dpkg -i removebroker_1.0.0_amd64.deb
```
...
```bash
# Windows (PowerShell)
Get-FileHash .\RemoveBroker-1.0.0-installateur.exe -Algorithm SHA256

# macOS et Linux
shasum -a 256 RemoveBroker-1.0.0-x64.AppImage
```
